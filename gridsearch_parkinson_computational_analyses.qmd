...
::: callout-note
check why id 111 is in model results but not behavioral data
:::
...
#df_params_performance$group == df_mean_reward_subject$group
...
p_lambda_vs_reward <- ggplot(df_params_performance, aes(x = lambda, y=mean_reward, group = ageGroup2, fill = ageGroup2, color = ageGroup2)) +
  geom_smooth(linetype = "dashed", size = 1, method="lm", se=F, alpha = 0.2) +
  geom_point(aes(fill = ageGroup2), shape = 21, alpha = 0.8, colour = "black") +
