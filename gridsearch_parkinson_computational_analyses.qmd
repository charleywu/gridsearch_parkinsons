...
p_gpucb_params <- ggplot(df_gpucb_params, aes(x = group, y = estimate, fill = group, shape = group, color = group)) +
  ...
  facet_wrap(
    ...
    labeller = labeller(
      param = c(
        ...
        "beta"   = "'directed exploration '*beta",
  ...
  scale_fill_manual(values = groupcolors) +
  scale_x_discrete("") + 
  ggtitle("GP-UCB parameter estimates") + 
  theme_classic() +
  theme(strip.background = element_blank(),  
        strip.text = element_text(color = "black", size=18),
        legend.position = "none",
...
### TO DO: 
1) make plots by group 
2) take ratio of explore/exploit and correlate with model params
3) make separate plots for params and log for beta and tau
...
## Overview figure computational results
