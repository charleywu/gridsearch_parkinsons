...
packages <- c('gridExtra', 'BayesFactor', 'tidyverse', "RColorBrewer", "lme4", "sjPlot", "lsr", "brms", "kableExtra", "afex", "emmeans", "viridis", "ggpubr", "hms", "scales", "cowplot", "gtsummary", "webshot", "webshot2", "parameters")
...
dat_gridsearch <- read_csv("data/data_gridsearch_Parkinson.csv", show_col_types = FALSE) %>% 
  mutate(type_choice  = as.factor(type_choice))

# get subject data
dat_sample <- read_delim("data/data_gridsearch_subjects.csv", delim = ";", escape_double = FALSE, trim_ws = TRUE, show_col_types = FALSE) %>% 
...
Participants were tested either shortly after taking their regular Levodopa (L-Dopa) dose (N=22) or just before their next scheduled dose (N=20). Patients with polyneuropathy served as a control group (N=20), comparable in age, depressive symptoms, and basic cognitive functioning.



We collected data from `r dat_sample %>% filter(group != "PNP") %>% nrow()` adult participants with Parkinson disease, who were tested either shortly after taking their regular L-Dopa dose (N=`r dat_sample %>% filter(group != "PD+") %>% nrow()`) or just before their next scheduled dose (N=`r dat_sample %>% filter(group != "PD-") %>% nrow()`). Patients with polyneuropathy served as a control group (N=`r dat_sample %>% filter(group == "PNP") %>% nrow()`), comparable in age, depressive symptoms, and basic cognitive functioning.
