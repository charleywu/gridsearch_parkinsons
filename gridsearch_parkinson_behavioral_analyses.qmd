...
>>>>>>> ee52d68455dab6c004425712200fb2c6393b6f5d
...
#| fig-cap: "Performance by group (excluding tutorial and bonus round)."
#| fig-width: 9
#| fig-height: 7
...
TO DO: make labels (unique, novel etc) consistent

```{r}
#| label: fig-choices
#| fig-cap: "Exploration and exploitation choices across all rounds. The inset shows the total proportion of exploration choices (unique tiles per round)."
#| fig-width: 9
#| fig-height: 5
...
p_main <- ggplot(dat_repeat_prop, aes(x = trial, y = prop_repeat, fill = group, shape = group, color = group)) +
  ...
  labs(
    x = "Trial",
    y = "Proportion of exploitation choices (%)",
    title = "Exploration and exploitation over time"
  ) +
  theme_classic() +
  theme(strip.background = element_blank(),  
        strip.text = element_text(color = "black", size=12),
        #legend.position = c(0.15, 1),  # Top-left
        legend.justification = c(0, 1),
        legend.title = element_blank()
  )

# Inset plot
p_inset <- ggboxplot(df_unique_choices_subject, 
                     x = "group", 
                     y = "m_prop_unique",
                     color = "group", palette = groupcolors, fill = "group", alpha = 0.2, size=0.2,
                     add = "jitter", jitter.size = 0.0, shape = "group", title = "Proportion exploration choices") +
  scale_y_continuous("Unique choices", 
                     breaks = c(0, 0.5, 1),  
  ...
  theme(strip.background = element_blank(),  
        ...
        plot.title = element_text(size = 0, hjust = 0.5, margin = margin(b = 0)), 
  )

# Combine plots
ggdraw() +
  draw_plot(p_main) +
  draw_plot(p_inset, x = 0.08, y = 0.52, width = 0.3, height = 0.4)

ggsave("plots/explore_exploit_choices.png", dpi=300, width = 9, height = 5)
...
#| fig-cap: "Prediction error of bonus round judgments."
#| fig-width: 6
#| fig-height: 5
#| 
