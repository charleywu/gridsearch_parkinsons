...
The comparison group (N=`r dat_sample %>% filter(group == "PNP") %>% nrow()`) consisted of individuals of similar age diagnosed with polyneuropathies (PNP), a condition affecting the peripheral nervous system that can lead to physical symptoms such as pain, sensory loss, or motor weakness. However, unlike Parkinson’s disease, PNP does not involve central dopaminergic dysfunction or cognitive impairment.
...
# PNP vs. PD+
# ttestBF(subset(df_mean_reward_subject, group == 'PNP')$mean_reward, subset(df_mean_reward_subject, group == 'PD+')$mean_reward, var.equal = TRUE)
...
The mean proportion of repeat choices (=exploitation) differed among all groups, with patients with polyneuropathy (PNP) showing higher levels of exploitation than both PD+ and PD- patients. Parkinson patients on medication (PD+) exploited more than patients off medication (PD-).

- PNP vs. PD+: `r ttestPretty(subset(df_unique_choices_subject, group == 'PNP')$m_prop_repeat, subset(df_unique_choices_subject, group == 'PD+')$m_prop_repeat, var.equal = TRUE)`
- PNP vs. PD-: `r ttestPretty(subset(df_unique_choices_subject, group == 'PNP')$m_prop_repeat, subset(df_unique_choices_subject, group == 'PD-')$m_prop_repeat, var.equal = TRUE)`
...
We also calculated the mean reward obtained from learners' explore and exploit choices, respectively. @fig-reward-explore-exploit shows that both PNP and PD+ patients obtained higher rewards from their exploration choices, indicating higher levels of adaptation to the structure of the environment. Similarly, for exploitative choices, PD+ and PNP patients obtained higher rewards, showing that PD− patients not only exploited less frequently but also did so less efficiently.
...
- PNP vs. PD+: `r ttestPretty(subset(df_reward_explore_exploit, group == 'PNP' & is_new_label == "Exploration")$mean_reward, subset(df_reward_explore_exploit, group == 'PD+' & is_new_label == "Exploration")$mean_reward, var.equal = TRUE)`
- PNP vs. PD-: `r ttestPretty(subset(df_reward_explore_exploit, group == 'PNP' & is_new_label == "Exploration")$mean_reward, subset(df_reward_explore_exploit, group == 'PD-' & is_new_label == "Exploration")$mean_reward, var.equal = TRUE)`
...
- PNP vs. PD+: `r ttestPretty(subset(df_reward_explore_exploit, group == 'PNP' & is_new_label == "Exploitation")$mean_reward, subset(df_reward_explore_exploit, group == 'PD+' & is_new_label == "Exploitation")$mean_reward, var.equal = TRUE)`
- PNP vs. PD-: `r ttestPretty(subset(df_reward_explore_exploit, group == 'PNP' & is_new_label == "Exploitation")$mean_reward, subset(df_reward_explore_exploit, group == 'PD-' & is_new_label == "Exploitation")$mean_reward, var.equal = TRUE)`
