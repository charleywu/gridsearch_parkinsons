...
![Screenshot from experiment, procedure and example environments](img/task_overview.png)
...
PD patients were randomly assigned to two conditions: on medication (PD on) and off medication (PD off). In the PD on group (N=`r dat_sample %>% filter(group == "PD on") %>% nrow()`), patients' scheduled L-Dopa was administered at least 30 minutes before the start of the experiment. In the PD off group (N=`r dat_sample %>% filter(group == "PD off") %>% nrow()`), the next scheduled dose for participants was timed such that they were in a low dopamine state during the experiment, offering a clear contrast to the PD on group. Thus, we refer to the ‘on medication’ condition as the state after taking L-Dopa and the ‘off medication’ condition as the state before their next scheduled dose.
...
@tbl-demographics shows the demographics of our sample, along with their Hoehn-Yahr, MMSE , and BDI scores. In the PD on group, the mean time since their last L-Dopa dose was `r dat_sample %>% filter(group == "PD on") %>% group_by(group) %>% summarise(mean_time_since_ldopa = mean(time_since_ldopa,  na.rm= T)) %>% pull(mean_time_since_ldopa) %>% round(0)` min; in the PD off group it was `r dat_sample %>% filter(group == "PD off") %>% group_by(group) %>% summarise(mean_time_since_ldopa = mean(time_since_ldopa,  na.rm= T)) %>% pull(mean_time_since_ldopa)%>% round(0)`min.
...
Participants’ learning curves (@fig-learning-curves) show the average reward obtained in each trial across rounds. For both polyneuropathy patients (Control) and PD patients on medication (PD on), the mean rewards increased as the round progresses, suggesting they effectively balanced exploration and exploitation to maximize rewards. In stark contrast, PD patients off medication (PD off) showed no improvement across trials.
...
We next examined choice behavior, focusing on the trade-off between exploration and exploitation. @fig-exploration-exploitation-choices shows that differences in reward accumulation are driven by learners’ ability to adequately balance exploration (selecting novel options) and exploitation (re-clicking tiles with known high rewards). PD patients tested on medication exploited more than patients off medication (`r ttestPretty(subset(df_unique_choices_subject, group == "PD on")$m_prop_repeat, subset(df_unique_choices_subject, group == "PD off")$m_prop_repeat, var.equal = TRUE)`), whose exploitation levels were notably low.

```{=html}
<!--
OLD TEXT
As a global measure we calculated the total proportion of exploitation choices (unique tiles, out of 25) across all rounds. Lower proportions indicate a greater tendency to explore novel options, while higher proportions suggest a preference for exploiting known options There were substantial differences between groups, with PD on patients showing higher levels of exploratory behavior and, conversely, a reduced tendency to exploit known options, compared to Control patients. This effect was particularly pronounced for PD off patients, who almost exclusively selected novels options during the task and only rarely exploited known options. These distinct behavioral patterns underlie the differences in performance, showing how suboptimal balance of exploration and exploitation affects obtained rewards. 

@fig-exploration-exploitation-choices shows that both controls and PD on patients increased the amount of exploitation over time, indicating a goal-directed shift from exploring novel options to exploiting known high-value options. The Control group began exploiting earlier in the round and exhibited a stronger overall tendency toward exploitation compared to the PD on group, indicating that this earlier focus on exploitation underlies their better performance. In stark contrast, PD off patients predominantly engaged in exploration and showed only a weak tendency towards exploitation as the search horizon approached its end. This pattern is also reflected in the overall proportion of exploitation decisions (@fig-exploration-exploitation-choices, inset). 

Control patients made more exploitation decisions than PD on patients (`r ttestPretty(subset(df_unique_choices_subject, group == "Control")$m_prop_repeat, subset(df_unique_choices_subject, group == "PD on")$m_prop_repeat, var.equal = TRUE)`), who exploited more than PD off patients (`r ttestPretty(subset(df_unique_choices_subject, group == "PD on")$m_prop_repeat, subset(df_unique_choices_subject, group == "PD off")$m_prop_repeat, var.equal = TRUE)`). Notably, PD off patients almost exclusively selected novels options during the task and only rarely exploited known options. These distinct behavioral patterns show how suboptimal balance of exploration and exploitation affects obtained rewards.
-->
```

## Exploitation choices

Controls exploited more than PD patients (`r ttestPretty(subset(df_unique_choices_subject, group == "Control")$m_prop_repeat, subset(df_unique_choices_subject, group == "PD on")$m_prop_repeat, var.equal = TRUE)`), but the difference was less pronounced than the comparison of the PD groups, aligning with their slightly higher levels of reward accumulation.
...
- Control vs. PD on: `r ttestPretty(subset(df_unique_choices_subject, group == 'Control')$m_prop_repeat, subset(df_unique_choices_subject, group == 'PD on')$m_prop_repeat, var.equal = TRUE)`
- Control vs. PD off: `r ttestPretty(subset(df_unique_choices_subject, group == 'Control')$m_prop_repeat, subset(df_unique_choices_subject, group == 'PD off')$m_prop_repeat, var.equal = TRUE)`
- PD on vs. PD off: `r ttestPretty(subset(df_unique_choices_subject, group == 'PD on')$m_prop_repeat, subset(df_unique_choices_subject, group == 'PD off')$m_prop_repeat, var.equal = TRUE)`

### Rewards obtained from exploration and exploitation
We also calculated the mean reward obtained from learners' explore and exploit choices, respectively. @fig-reward-explore-exploit shows that both controls and PD on patients obtained higher rewards from their exploration choices, indicating higher levels of adaptation to the structure of the environment. Similarly, for exploitative choices, PD on and Control patients obtained higher rewards, showing that PD off patients not only exploited less frequently but also did so less efficiently.
...
- Control vs. PD on: `r ttestPretty(subset(df_reward_explore_exploit, group == 'Control' & is_new_label == "Exploration")$mean_reward, subset(df_reward_explore_exploit, group == 'PD on' & is_new_label == "Exploration")$mean_reward, var.equal = TRUE)`
- Control vs. PD off: `r ttestPretty(subset(df_reward_explore_exploit, group == 'Control' & is_new_label == "Exploration")$mean_reward, subset(df_reward_explore_exploit, group == 'PD off' & is_new_label == "Exploration")$mean_reward, var.equal = TRUE)`
- PD on vs. PD off: `r ttestPretty(subset(df_reward_explore_exploit, group == 'PD on' & is_new_label == "Exploration")$mean_reward, subset(df_reward_explore_exploit, group == 'PD off' & is_new_label == "Exploration")$mean_reward, var.equal = TRUE)`


_Exploit choices_

- Control vs. PD on: `r ttestPretty(subset(df_reward_explore_exploit, group == 'Control' & is_new_label == "Exploitation")$mean_reward, subset(df_reward_explore_exploit, group == 'PD on' & is_new_label == "Exploitation")$mean_reward, var.equal = TRUE)`
- Control vs. PD off: `r ttestPretty(subset(df_reward_explore_exploit, group == 'Control' & is_new_label == "Exploitation")$mean_reward, subset(df_reward_explore_exploit, group == 'PD off' & is_new_label == "Exploitation")$mean_reward, var.equal = TRUE)`
- PD on vs. PD off: `r ttestPretty(subset(df_reward_explore_exploit, group == 'PD on' & is_new_label == "Exploitation")$mean_reward, subset(df_reward_explore_exploit, group == 'PD off' & is_new_label == "Exploitation")$mean_reward, var.equal = TRUE)`
...
p_distance_consecutive <- ggplot(df_props, aes(x = distance, y = prop, color = group)) +
  ...
  geom_col(
    ...
    # fill     = "firebrick",
    fill   = highlight_colours[ df_props$group[df_props$distance == 0] ],
    color    = "black",
    width    = 1
  ) +
  scale_fill_manual(values = groupcolors) +
...
### Search distances: All choices
 - Control vs. PD+  patients on medication: `r ttestPretty(subset(df_mean_distance_subject, group == "Control")$mean_distance, subset(df_mean_distance_subject, group == "PD on")$mean_distance, var.equal = TRUE)` 
 - Control vs. PD- off medication: `r ttestPretty(subset(df_mean_distance_subject, group == "Control")$mean_distance, subset(df_mean_distance_subject, group == "PD off")$mean_distance, var.equal = TRUE)`
 - PD+ on medication vs. PD- off medication: `r ttestPretty(subset(df_mean_distance_subject, group == "PD on")$mean_distance, subset(df_mean_distance_subject, group == "PD off")$mean_distance, var.equal = TRUE)`
...
For this analysis, _exploratory choices_ are defined as consecutive selections in which the previously chosen tile is not clicked again (distance = 0), but a different tile farther away is selected (distance > 0).
 ...
 - Control vs. PD+  patients on medication: `r ttestPretty(subset(df_mean_distance_subject_exploration, group == "Control")$mean_distance, subset(df_mean_distance_subject_exploration, group == "PD on")$mean_distance, var.equal = TRUE)`
 - Control vs. PD- off medication: `r ttestPretty(subset(df_mean_distance_subject_exploration, group == "Control")$mean_distance, subset(df_mean_distance_subject_exploration, group == "PD off")$mean_distance, var.equal = TRUE)`
 - PD+ on medication vs. PD- off medication: `r ttestPretty(subset(df_mean_distance_subject_exploration, group == "PD on")$mean_distance, subset(df_mean_distance_subject_exploration, group == "PD off")$mean_distance, var.equal = TRUE)`


```{r echo=F}

### Mean search distance: Control vs. PD on
# ttestPretty(subset(df_mean_distance_subject, group == "Control")$mean_distance, subset(df_mean_distance_subject, group == "PD on")$mean_distance, var.equal = TRUE)
# t.test(subset(df_mean_distance_subject, group == "Control")$mean_distance, subset(df_mean_distance_subject, group == "PD on")$mean_distance, var.equal = TRUE)
# ttestBF(subset(df_mean_distance_subject, group == "Control")$mean_distance, subset(df_mean_distance_subject, group == "PD on")$mean_distance, var.equal = TRUE)

### Mean search distance: Control vs. PD off
# ttestPretty(subset(df_mean_distance_subject, group == "Control")$mean_distance, subset(df_mean_distance_subject, group == "PD off")$mean_distance, var.equal = TRUE)
# t.test(subset(df_mean_distance_subject, group == "Control")$mean_distance, subset(df_mean_distance_subject, group == "PD off")$mean_distance, var.equal = TRUE)
# ttestBF(subset(df_mean_distance_subject, group == "Control")$mean_distance, subset(df_mean_distance_subject, group == "PD off")$mean_distance, var.equal = TRUE)


### Mean search distance: PD on vs. PD off
# ttestPretty(subset(df_mean_distance_subject, group == "PD on")$mean_distance, subset(df_mean_distance_subject, group == "PD off")$mean_distance, var.equal = TRUE)
# t.test(subset(df_mean_distance_subject, group == "PD on")$mean_distance, subset(df_mean_distance_subject, group == "PD off")$mean_distance, var.equal = TRUE)
# ttestBF(subset(df_mean_distance_subject, group == "PD on")$mean_distance, subset(df_mean_distance_subject, group == "PD off")$mean_distance, var.equal = TRUE)
```
...
Across all trials and rounds, search distance and previous reward were negatively correlated, indicating that participants tended to search further away following lower rewards compared to higher rewards. This relationship was stronger in Control patients (`r corTestPretty(subset(dat, group == "Control" & trial != 0 & round %in% 2:9)$previous_reward, subset(dat, group == "Control" & trial != 0 & round %in% 2:9)$distance)`) and PD on patients (`r corTestPretty(subset(dat, group == "PD on" & trial != 0 & round %in% 2:9)$previous_reward, subset(dat, group == "PD on" & trial != 0 & round %in% 2:9)$distance)`) compared to PD off patients (`r corTestPretty(subset(dat, group == "PD off" & trial != 0 & round %in% 2:9)$previous_reward, subset(dat, group == "PD off" & trial != 0 & round %in% 2:9)$distance)`). These findings suggest that PD patients off medication exhibited less adaptive search behavior than those on medication and individuals with polyneuropathies.
...
* PD on: `r corTestPretty(subset(dat, group == "PD on" & trial != 0 & round %in% 2:9)$previous_reward, subset(dat, group == "PD on" & trial != 0 & round %in% 2:9)$distance)`
* PD off: `r corTestPretty(subset(dat, group == "PD off" & trial != 0 & round %in% 2:9)$previous_reward, subset(dat, group == "PD off" & trial != 0 & round %in% 2:9)$distance)`
-->
```
...
-   PD on: `r ttestPretty(subset(dat_bonus_error, group=='PD on')$mean_error, mu =randomPerformance)`
-   PD off: `r ttestPretty(subset(dat_bonus_error, group=='PD off')$mean_error, mu =randomPerformance)`

Controls had lower prediction error than PD off patients; no other group differences were observed.

-   Control vs. PD on: `r ttestPretty(subset(dat_bonus_error, group == 'Control')$mean_error, subset(dat_bonus_error, group == 'PD on' )$mean_error, var.equal = T)`
-   Control vs. PD off: `r ttestPretty(subset(dat_bonus_error, group == 'Control')$mean_error, subset(dat_bonus_error, group == 'PD off' )$mean_error, var.equal = T)`
-   PD on vs. PD off: `r ttestPretty(subset(dat_bonus_error, group == 'PD on')$mean_error, subset(dat_bonus_error, group == 'PD off' )$mean_error, var.equal = T)`
...
# ranktestPretty(subset(dat_bonus_error, group == 'Control')$mean_error, subset(dat_bonus_error, group == 'PD on' )$mean_error)
...
```{r echo = F}
# t.test(subset(dat_bonus_error, group == "PD on")$mean_error, subset(dat_bonus_error, group == "PD off")$mean_error, var.equal = T)
# ttestBF(subset(dat_bonus_error, group == "PD on")$mean_error, subset(dat_bonus_error, group == "PD off")$mean_error)

# t.test(subset(dat_bonus_error, group == "Control")$mean_error, subset(dat_bonus_error, group == "PD on")$mean_error, var.equal = T)
# ttestBF(subset(dat_bonus_error, group == "Control")$mean_error, subset(dat_bonus_error, group == "PD on")$mean_error)

# t.test(subset(dat_bonus_error, group == "Control")$mean_error, subset(dat_bonus_error, group == "PD off")$mean_error, var.equal = T)
# ttestBF(subset(dat_bonus_error, group == "Control")$mean_error, subset(dat_bonus_error, group == "PD off")$mean_error)
```
...
-   PD on: `r corTestPretty(subset(dat_bonus, group == "PD on")$error, subset(dat_bonus, group == "PD on")$howSecure, method = "pearson")`
-   PD off: `r corTestPretty(subset(dat_bonus, group == "PD off")$error, subset(dat_bonus, group == "PD off")$howSecure, method = "pearson")`
...
xseq   <- seq(0,10, 1)
newdat <- data.frame(group = rep(c("Control","PD on", "PD off"), each=length(xseq)), howSecure = rep(xseq,3))
preds  <- fitted(brm_bonus_confidence_error_by_group, re_formula = NA, newdata = newdat, probs = c(0.025, 0.975))

fixedDF_conf_error <- data.frame(group = rep(c("Control","PD on", "PD off"), each=length(xseq)), howSecure = rep(xseq,3), error = preds[,1], lower = preds[,3], upper = preds[,4] )
...
-   PD on: `r ttestPretty(subset(df_chosen_group_wide, group == 'PD on')$chosen, subset(df_chosen_group_wide, group == 'PD on' )$not_chosen, paired = T)`
-   PD off: `r ttestPretty(subset(df_chosen_group_wide, group == 'PD off')$chosen, subset(df_chosen_group_wide, group == 'PD off' )$not_chosen, paired = T)`
