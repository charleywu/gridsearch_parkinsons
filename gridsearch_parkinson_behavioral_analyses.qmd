...
We also calculated the mean reward obtained from learners' explore and exploit choices, respectively. @fig-reward-explore-exploit shows that both Control and PD on patients obtained higher rewards from their exploration choices, indicating higher levels of adaptation to the structure of the environment. Similarly, for exploitative choices, PD on and Control patients obtained higher rewards, showing that PD off patients not only exploited less frequently but also did so less efficiently.
...
p_distance_consecutive <- ggplot(filter(dat, trial > 0 & round %in% 2:9), aes(x = distance)) +
  facet_wrap(~group) +
...
Control patients had lower search distances than the PD on group (`r ttestPretty(subset(df_mean_distance_subject, group == "Control")$mean_distance, subset(df_mean_distance_subject, group == "PD on")$mean_distance, var.equal = TRUE)`) and lower distances than the PD off group (`r ttestPretty(subset(df_mean_distance_subject, group == "Control")$mean_distance, subset(df_mean_distance_subject, group == "PD off")$mean_distance, var.equal = TRUE)`). There was no difference between Parkinson patient with (PD on) and without (PD off) medication (`r ttestPretty(subset(df_mean_distance_subject, group == "PD on")$mean_distance, subset(df_mean_distance_subject, group == "PD off")$mean_distance, var.equal = TRUE)`).
...
p_bonusround_prediction_chosen <-
  ...
  scale_color_manual(values = groupcolors) +
  scale_fill_manual(values = groupcolors) +
  labs(title = "Predicted Reward: chosen vs. not chosen", y = "Predicted reward", x = "") +
  theme_classic() +
  theme(
    legend.position = 'none',
    legend.title = element_blank(),
...
ggsave("plots/behavioral_results.png", dpi=300, height=20, width=17)
ggsave("plots/behavioral_results.pdf", height=20, width=17)
