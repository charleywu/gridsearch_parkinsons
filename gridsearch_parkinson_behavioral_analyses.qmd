...
packages <- c('gridExtra', 'BayesFactor', 'tidyverse', "RColorBrewer", "lme4", "sjPlot", "lsr", "brms", "kableExtra", "afex", "emmeans", "viridis", "ggpubr", "hms", "scales", "cowplot", "gtsummary", "webshot", "webshot2", "parameters", "bridgesampling", "cmdstanr")
...
ggsave("plots/p_explore_exploit_choices_over_time.png", dpi=300, width = 8, height = 4.5)
...
We also calculated the mean reward obtained from learners' explore and exploit choices, respectively. @fig-reward-explore-exploit shows that both Control and PD+ patients obtained higher rewards from their exploration choices, indicating higher levels of adaptation to the structure of the environment. Similarly, for exploitative choices, PD+ and Control patients obtained higher rewards, showing that PD- patients not only exploited less frequently but also did so less efficiently.
...
ggsave("plots/distance.png", p_distance_consecutive, dpi=300,height=3, width=5)
ggsave("plots/distance.pdf", p_distance_consecutive, height=3, width=5)
```

Control patients had lower search distances than the PD+ group (`r ttestPretty(subset(df_mean_distance_subject, group == "Control")$mean_distance, subset(df_mean_distance_subject, group == "PD+")$mean_distance, var.equal = TRUE)`) and lower distances than the PD- group (`r ttestPretty(subset(df_mean_distance_subject, group == "Control")$mean_distance, subset(df_mean_distance_subject, group == "PD-")$mean_distance, var.equal = TRUE)`). There was no difference between Parkinson patient with (PD+) and without (PD-) medication (`r ttestPretty(subset(df_mean_distance_subject, group == "PD+")$mean_distance, subset(df_mean_distance_subject, group == "PD-")$mean_distance, var.equal = TRUE)`).
...
ggplot(dat_bonus_error, aes(x = group, y = mean_error, color = group, fill = group, shape = group)) +
  ...
  # stat_compare_means(
  #   comparisons = list(c("Control", "PD+"), c("PD+", "PD-"), c("Control", "PD-")),
  #   paired = FALSE,
  #   method = "t.test",
  #   aes(label = paste0("p = ", after_stat(p.format)))
  # ) +
...
- relate difference confidence chosen not chosen relate to individual beta
...
# Appendix
## Overview figure behavioral results
...
#| fig-height: 15
#| 
cowplot::plot_grid(p_performance, p_learning_curves, p_explore_exploit_choices, p_explore_exploit_choices_over_time, p_distance_consecutive, p_regression_distance_reward,  labels = c('A', 'B','C', 'D', 'E', 'F'), label_size = 20, nrow = 3)
...
ggsave("plots/behavioral_results.png", dpi=300, height=16, width=15)
ggsave("plots/behavioral_results.pdf", height=16, width=15)
