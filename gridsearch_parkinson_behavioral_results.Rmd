---
title: "Gridsearch Parkinson"
author: "Björn Meder, Martha Sterf, Charley M. Wu, Matthias Guggenmos"
...
# get random performance for reward 
# df_random_performance <- read.csv2('data/rand_performance.csv')
```
...
![Screenshot from experiment and example environments (from Giron et al. 2023)](img/task_overview_Giron_et_al_2021.png)
...
## Sample
We collected data from adult participants with Parkinson's disease (PD) who receive Levodopa (L-Dopa; Abbott, 2010) for treatment. L-Dopa is a pharmacological agent used to treat the symptoms of Parkinson’s disease, which is associated with a deficiency of dopamine. Since dopamine cannot pass the blood-brain barrier, L-Dopa is used as a precursor, which is absorbed through the gastrointestinal tract and subsequently metabolized to dopamine. 

PD patients were randomly assigned to two conditions: on medication (PD+) and off medication (PD-). In the PD+ group (_n_=22), L-Dopa was administered at least 30 minutes before the start of the experiment to allow the drug sufficient time to take effect during the task. In the PD- group (_n_=20), the next scheduled dose for participants was timed such that they were in a low dopamine state during the experiment, offering a clear contrast to the PD+ group.

Patients with polyneuropathies (PNP) served as control group. The PNP group (_n_=20) consisted of individuals with polyneuropathies, which is associated with physical symptoms similar to the motor impairments seen in Parkinson’s disease. However, since polyneuropathy primarily affects peripheral nerves it is typically not associated with cognitive impairments, enabling a comparison in terms of physical symptomatology and the resulting burden of suffering.

Possible participants with Parkinson’s disease were evaluated based on Hoehn-Yahr scores recorded in their patient files. The scale, which ranges from 1 to 5, assesses disease severity and motor impairments, with higher scores indicating greater severity (Hoehn & Yahr, 1967; Goetz et al., 2004). We limited recruitment to individuals with scores between 1 and 3, as scores of 4 and 5 reflect severe impairment.

## Questionnaires
### Beck's depression inventory II (BDI-II)
All participants answered the German version of the Beck Depression Inventory II, a self-report inventory consisting of 21 items measuring the severity of depression (Beck et al., 2011; Hauzinger et al., 2006). Each item is scored from 0 to 3, with higher values representing more severe symptoms, such that the total score ranges between 0 and 63. A score of 0-11 is considered normal, 12-19 points indicate mild depressive symptoms, while values of 20 or higher suggest moderate to severe depression. (TO DO: check BDI scoring). 
...
```{r}
#read in data
# dat       <- read.table(file="data/data_gridsearch_parkinson.csv", header = T, sep=",", stringsAsFactors = T) # data generated from csv

# dat       <- read_delim("data/ALLE DATEN CSV.csv",
...
# NOTE: id=495 has Hoehn Yahr value of 65 (should be 0-3)
...
# subjects  <- read.table(file="data/subject_data.csv", header = T, sep=",", stringsAsFactors = T)
# subjects <- subjects %>%
#   select(-Pseudonym) %>%
#   mutate(gender = str_trim(gender)) %>%
#   mutate(gender = recode(gender, "w" = "f", "m" = "m")) %>%
#   mutate(gender = as.factor(gender))

# # subject ids from BA thesis. has exclude_ids <- c(1, 2, 16), where do they come from?
# dat_BA <- read.csv2(file="data/Datensatz zur Auswertung 2.csv", header = T, sep=";", stringsAsFactors = T) 
# length(dat_BA$id)

# NOTES BM
# für id 11 gibt es ein JSOn file namens 111. id 11 taucht in ALLE DATEN.csv nicht auf

# wo kommen in dat_BA / ids 1, 2, 16 her? (die ausgeschlossen werden)
...
dodge <- position_dodge(width = 0.3)  # Adjust the width to control dodging
...
ggplot(df_summary_by_round, aes(x = round, y = mean_of_means, group = group, shape = group,  color = group, fill = group)) +
  ...
  geom_errorbar(aes(ymin = mean_of_means - se_reward, ymax = mean_of_means + se_reward), width = 0.2, position = dodge, alpha=0.7) +  #
  geom_point(position = dodge, size = 3, stroke = 1, alpha=.9) +  
...
kable(as.data.frame(aov_rounds$anova_table), format = "html", escape = F, digits = 2, caption = "ANOVA results with round as within-subjects factor and group as between subjects factor, where rewards per round were first aggregated within subjects.") %>%
  kable_styling("striped", full_width = F)
...
##########################################
# Boxplots of rewards for the groups
##########################################
ggboxplot(df_mean_reward_subject, 
          x = "group", 
          y = "mean_reward",
          color = "group", palette =mycolors, fill = "group", alpha = 0.2,
          add = "jitter", jitter.size = 0.5, shape = "group", title = "Performance") +
  ylab("Reward") +
  xlab("") +
...
- PNP vs. PD+: `r ttestPretty(subset(df_mean_reward_subject, group == 'PNP')$mean_reward, subset(df_mean_reward_subject, group == 'PD+' )$mean_reward, var.equal = T)`
- PNP vs. PD-: `r ttestPretty(subset(df_mean_reward_subject, group == 'PNP')$mean_reward, subset(df_mean_reward_subject, group == 'PD-' )$mean_reward, var.equal = T)`
- PD+ vs. PD-: `r ttestPretty(subset(df_mean_reward_subject, group == 'PNP')$mean_reward, subset(df_mean_reward_subject, group == 'PD+' )$mean_reward, var.equal = T)`
...
ggsave("plots/learning_curves.png", dpi=300,height=5,width=6)
...
ggplot(filter(dat, trial > 0 & round %in% 2:9), aes(x = distance)) +
  facet_wrap(~group) +
  geom_histogram(aes(y=..density.., fill = group), binwidth = 1) +
...
There was no difference between Parkinson patient with (PD+) and without (PD-) medication, `r ttestPretty(subset(df_mean_distance_subject, group == "PD+")$mean_distance, subset(df_mean_distance_subject, group == "PD-")$mean_distance, var.equal = TRUE)`
...
dat_bonus           <- read.csv(file="data/data_gridsearch_Parkinson_bonusround.csv", header = T, sep=",", stringsAsFactors = T)
dat_bonus <- dat_bonus %>% 
...
dat_bonus <- dat_bonus %>% left_join(dat %>% select(id, group) %>% distinct(id, .keep_all = TRUE), by = "id")
...
ggboxplot(dat_bonus_error, 
          x = "group", 
          y = "mean_error",
          color = "group", palette = mycolors, fill = "group", alpha = 0.2,
          add = "jitter", jitter.size = 0.5, shape = "group", title = "Prediction error") +
...
# ggplot(dat_bonus_error, aes(x=group, y=mean_error, group = group, fill = group)) +
#   geom_hline(yintercept = randomPerformance, linetype = 'dotted') +
#   geom_boxplot(outlier.shape = NA, width = w_box) +
#   stat_summary(fun = mean, geom="point", shape = 23, fill = "white", size=1.8) +   
#   geom_jitter(aes(x = as.numeric(group) +  line_jitter + 0.15,  colour=group),  shape = 21, size = 1.3, height = jit_height, width = jit_width, alpha = jit_alpha, colour = "black") +
#   scale_fill_manual(values=mycolors) +
#   scale_y_continuous("Mean absolute error", limits = c(0,35)) +
#   scale_x_discrete("") +
#   ggtitle("Prediction error of age groups") + 
#   theme_classic() +
#   theme(aspect.ratio = 1,
#         plot.title = element_text(hjust = 0.5),
#         legend.title = element_blank(),
...
### Performance as function of depression score (BDI)
The plots show performance as function of depression score (BDI-II), separately for each group. Opposing trends were found in the different groups: for patients with polyneuropathy (PNP), there was a negative relation such that patients with higher depression scores obtained _lower_ rewards. For the two Parkinson groups, the relation was positive, such that patients reporting more severe symptoms obtained _higher_ rewards. 

```{r echo=FALSE, fig.width= 10, fig.height=5}
# df_correlation_BDI <- df_mean_reward_subject %>%  summarise(correlation = cor(BDI, mean_reward, use = "complete.obs"))
# 
...
#   coord_cartesian(xlim = c(0,15), ylim = c(23,40)) +
#   ggtitle("Performance as function of depression score") +
#   geom_text(data = df_correlation_BDI, aes(x = 1, y = 23, label = paste0("italic(r) == ", round(correlation, 2))), parse = TRUE, inherit.aes = FALSE) +
#   theme_classic() +
#   theme(aspect.ratio = 1,
#         plot.title = element_text(hjust = 0.5),
#         legend.title = element_blank(),
...
#         panel.grid.minor = element_blank())
# 
# ggsave("plots/performance_BDI.png", dpi=300, height = 4, width = 4.5)
...
```{r echo=FALSE, fig.width= 10, fig.height=5}
# df_correlation_MMSE <- df_mean_reward_subject %>%  summarise(correlation = cor(MMSE, mean_reward, use = "complete.obs"))
# 
# ggplot(df_mean_reward_subject, aes(x = MMSE, y = mean_reward)) +
#   #geom_hline(data=filter(df_random_performance, environment=="Rough"), linetype="dotted", aes(yintercept=z_learn_envs)) + 
#   #geom_hline(data=filter(df_random_performance, environment=="Smooth"), linetype="dotted",aes(yintercept=z_learn_envs)) +
#   geom_smooth(colour = "black", linetype = "dashed", size = 0.5, method="lm", se=T, alpha = 0.2) +
#   # geom_jitter(aes(fill = MMSE), shape = 21, alpha = jit_alpha, colour = "black") +
...
#   geom_text(data = df_correlation_MMSE, aes(x = 27.2, y = 23, label = paste0("italic(r) == ", round(correlation, 2))), parse = TRUE, inherit.aes = FALSE) +
#   theme_classic() +
#   theme(aspect.ratio = 1,
#         plot.title = element_text(hjust = 0.5),
#         legend.title = element_blank(),
...
#         panel.grid.minor = element_blank())
# 
# ggsave("plots/performance_MMSE.png", dpi=300, height = 4, width = 4.5)
...
```{r echo=FALSE, fig.width= 10, fig.height=5}
# df_correlation_hoehn_yahr <- df_mean_reward_subject %>%  
#   filter(id != 495) %>%  # has value of 65, needs correction
#   filter(group != "PNP") %>% 
#   summarise(correlation = cor(hoehn_yahr, mean_reward, use = "complete.obs"))
# 
# ggplot(filter(df_mean_reward_subject, id!=495 & group != "PNP"), aes(x = hoehn_yahr, y = mean_reward)) +
#   #geom_hline(data=filter(df_random_performance, environment=="Rough"), linetype="dotted", aes(yintercept=z_learn_envs)) + 
#   #geom_hline(data=filter(df_random_performance, environment=="Smooth"), linetype="dotted",aes(yintercept=z_learn_envs)) +
#   geom_smooth(colour = "black", linetype = "dashed", size = 0.5, method="lm", se=T, alpha = 0.2) +
#   # geom_jitter(aes(fill = MMSE), shape = 21, alpha = jit_alpha, colour = "black") +
...
#   geom_text(data = df_correlation_hoehn_yahr, aes(x = 1.2, y = 23, label = paste0("italic(r) == ", round(correlation, 2))), parse = TRUE, inherit.aes = FALSE) +
#   theme_classic() +
#   theme(aspect.ratio = 1,
#         plot.title = element_text(size=16),
#         legend.title = element_blank(),
...
#         panel.grid.minor = element_blank())
# 
# ggsave("plots/performance_HY.png", dpi=300, height = 4, width = 4.5)
...
# Bibliography
Abbott, A. (2010). Levodopa: the story so far. _Nature_, _466_(7310), S6-S7.

Beck, A. T., Steer, R. A., & Brown, G. (2011). Beck Depression Inventory–II. https://doi.org/10.1037/t00742-000

Folstein, M. F., Folstein, S. E., & McHugh, P. R. (1975). “Mini-Mental State”: A practical method for grading the cognitive state of patients for the clinician. _Journal of Psychiatric Research_, _12_(3), 189-198. https://doi.org/10.1016/0022-3956(75)90026-6

Hautzinger, M., Keller, F., & Kühner, C. (2006). Beck Depressions-Inventar (BDI-II). Revision. Frankfurt/Main: Harcourt Test Services.

Hoehn M, Yahr M (1967). Parkinsonism: onset, progression and mortality. _Neurology._ _17_(5): 427–42. https://doi.org/10.1212/wnl.17.5.427

Goetz, C. G., Poewe, W., Rascol, O., Sampaio, C., Stebbins, G. T., Counsell, C., ... & Seidl, L. (2004). Movement Disorder Society Task Force report on the Hoehn and Yahr staging scale: status and recommendations. _Movement Disorders_, _19_(9), 1020-1028.

Oertel, W. H., Berardelli, A., Bloem, B. R., Bonnet, A. M., Deuschl, G., & Kanovsky, P. (2012). Parkinson-Syndrome und andere Bewegungsstörungen. In W. H. Oertel, A. Berardelli, B. R. Bloem, A. M. Bonnet, G. Deuschl, & P. Kanovsky (Hrsg.), _30 Skalen zur Beurteilung von Schweregrad und Beeinträchtigung bei Bewegungsstörungen_ (S. 587). Thieme.

Tambasco, N., Romoli, M., & Calabresi, P. (2018). Levodopa in Parkinson's disease: current status and future developments. _Current Neuropharmacology_, _16_(8), 1239-1252.
