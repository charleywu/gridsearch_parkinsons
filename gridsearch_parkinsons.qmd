...
```{r}
# regression: judgment error as function of clinial variables

dat_bonus_error <- dat_bonus_error %>% 
  left_join(
    dat_sample %>% select(id, BDI, MMSE, hoehn_yahr),
    by = "id"
  )

lm_error_PD_only_BDI_MMSE_HY <- lm(mean_error ~ group + BDI + MMSE + hoehn_yahr, 
                                        data = subset(dat_bonus_error, group!='Control'))

# summary(lm_error_PD_only_BDI_MMSE_HY)

tab_model(lm_error_PD_only_BDI_MMSE_HY, title = "Regression results: Judgment error of PD patients in bonus round as function of BDI, MMSE, and Hoehn-Yahr score.")
