...
Data frame `dat` includes all info from `data_gridsearch_parkinson.csv` and `data_gridsearch_subjects.csv`, with the following variables (@tbl-dat):

-   `id`: participant id
-   `age` is participant age in years
-   `gender`: (m)ale, (f)emale, (d)iverse
-   `group`: condition, where PD+ = PArkinson patients _on_ medication, PD- = Parkinson patients _off_ medication, and Control = age-matched control patients with polyneurpathies
-   `BDI`: Depression score according to the German verison of Beck's Depression Inventory II
-   `MMSE`: Score on Mini Mental Status Examination
-   `LDD`: Daily Levodopa dose (only patients who consented to the access of their medical records; 14 PD$-$ and 12 PD$+$ patients)
-   `LEDD`: Levodopa equivalent daily dose, calculated according to Jost et al. (2023) 
-   `hoehn-yahr`: PD severity according to the Hoehn-Yahr scale
...
-   `time`is the timestamp of clicks
...
# compute latency between choices
dat_gridsearch <-  dat_gridsearch %>%
  arrange(id, round, trial) %>%
  group_by(id, round) %>%
  mutate(
    latency_ms = time - lag(time)
  ) %>%
  ungroup()
...
# get medication info (note: not available for all PD patients)
dat_medication <- read_csv(file="data/levodopa.csv", col_select = -1) 

dat_sample <- dat_sample %>% 
  left_join(dat_medication, by = "id")
...
dat_sample %>%
  ...
  select(group, gender, age, BDI, MMSE, hoehn_yahr, LEDD, time_since_ldopa) %>%
  tbl_summary(
    by = group,
    type = list(
      gender                                          ~ "categorical",
      c(age, BDI, MMSE, hoehn_yahr, LEDD, time_since_ldopa)  ~ "continuous"
    ),
    label = list(
      age        ~ "Age",
...
df_reward_subset_both_explot_explore <- df_reward_explore_exploit_full %>% 
  group_by(id) %>% 
  filter(all(n[is_new_label %in% c("Exploration", "Exploitation")] > 0)) 
...
# Analysis including all subjects, meaning that subjects who never exploited are still considered in the explore decisions:
# 
# _Explore choices_
# 
...
Group comaprisons including only subjects who engaged at least once in each choice type:

_Explore choices_

- PD+ vs. PD-: `r ttestPretty(subset(df_reward_subset_both_explot_explore, group == 'PD+' & is_new_label == "Exploration")$mean_reward, subset(df_reward_subset_both_explot_explore, group == 'PD-' & is_new_label == "Exploration")$mean_reward, var.equal = TRUE)`
- Control vs. PD-: `r ttestPretty(subset(df_reward_subset_both_explot_explore, group == 'Control' & is_new_label == "Exploration")$mean_reward, subset(df_reward_subset_both_explot_explore, group == 'PD-' & is_new_label == "Exploration")$mean_reward, var.equal = TRUE)`
- Control vs. PD+: `r ttestPretty(subset(df_reward_subset_both_explot_explore, group == 'Control' & is_new_label == "Exploration")$mean_reward, subset(df_reward_subset_both_explot_explore, group == 'PD+' & is_new_label == "Exploration")$mean_reward, var.equal = TRUE)`

_Exploit choices_

- PD+ vs. PD-: `r ttestPretty(subset(df_reward_subset_both_explot_explore, group == 'PD+' & is_new_label == "Exploitation")$mean_reward, subset(df_reward_subset_both_explot_explore, group == 'PD-' & is_new_label == "Exploitation")$mean_reward, var.equal = TRUE)`
- Control vs. PD-: `r ttestPretty(subset(df_reward_subset_both_explot_explore, group == 'Control' & is_new_label == "Exploitation")$mean_reward, subset(df_reward_subset_both_explot_explore, group == 'PD-' & is_new_label == "Exploitation")$mean_reward, var.equal = TRUE)`
- Control vs. PD+: `r ttestPretty(subset(df_reward_subset_both_explot_explore, group == 'Control' & is_new_label == "Exploitation")$mean_reward, subset(df_reward_subset_both_explot_explore, group == 'PD+' & is_new_label == "Exploitation")$mean_reward, var.equal = TRUE)`
...
Given the nested structure of the data, we next employed a Bayesian hierarchical regression analysis to predict search distance based on the reward obtained in the previous step, with group and their interaction as population-level (fixed) effects and subject-wise random intercepts. These analyses show that both the magnitude of reward obtained on the last step and group influence search distance. Notably, PD patients off medication (PD-) adapted their search behavior less in response to reward magnitude, while patients on medication (PD+) exhibited adaptation levels close to controls (@fig-brm_distance_reward and @tbl-brm_distance_reward-exploration).
...
```{r}
#| label: tbl-brm_distance_reward-exploration
#| tbl-cap: "Bayesian regression results: Search distance as function of reward on previous step." 

# create HTML table
tab_model(brm_distance_reward, bpe="mean")
...
ggsave("plots/Fig1_task_model.png", bg="white", dpi=300, height=10, width=15)
...
ggsave("plots/Fig2_behavioral_results.png", bg="white", dpi=300, height=15, width=18)
...
ggsave("plots/Fig3_computational_results.png", fig3_computational_results, bg="white", dpi=300, height=12, width=15)
...
#### No medication-related difference in decision latency 
To examine whether the behavioral differences between PD+ and PD- patients could reflect a general slowing of responding under dopamine depletion, we conducted exploratory analyses of response time, defined as the latency between consecutive clicks on the grid. @fig-latency shows the median response time across all trials and rounds by group. 

```{r}
#| label: fig-latency
#| fig-cap: "Median response time between consecutive clicks across all trials and rounds."
#| fig-width: 9
#| fig-height: 4


# compute latency per subject, pooling all decisions across rounds 2-9
df_latency_subject <- dat %>%
  filter(trial > 0 & round %in% 2:9) %>%
  group_by(id, group) %>%
  summarise(decisions=n(),
            mean_latency_ms = mean(latency_ms, na.rm = TRUE),
            median_latency_ms = median(latency_ms),
            SD_latency_ms = sd(latency_ms),            
            SE_latency_ms = SD_latency_ms / sqrt(decisions),
            .groups = "drop") 
...
df_latency_subject %>%
  ...
  summarise(
    n = n(),
    median_of_subject_medians_ms = median(median_latency_ms, na.rm = TRUE),
    mean_of_subject_medians_ms   = mean(median_latency_ms, na.rm = TRUE),
    sd_of_subject_medians_ms     = sd(median_latency_ms, na.rm = TRUE),
    se_of_subject_medians_ms     = sd_of_subject_medians_ms / sqrt(n),
    .groups = "drop"
  )


ggplot(df_latency_subject, aes(x = median_latency_ms/1000, fill = group, color = group)) +
  facet_wrap(~group, nrow = 1) +
  geom_histogram(aes(y = after_stat(count / sum(count))),
                 binwidth = 1, 
                 boundary = 0,
                 alpha = 0.7) +
  scale_y_continuous("Proportion of subjects", labels = percent_format(), expand = c(0, 0)) +
  scale_x_continuous("Median latency between choices (seconds)") +
  scale_fill_manual(values = groupcolors) +
  scale_color_manual(values = groupcolors) +
  theme_classic() +
  theme(
...
ggsave("plots/S2_decision_latency.png", dpi=300, width=9, height=4)
```

PD patients on and off medication did not differ in their median response time, and patients on medication did not differ from controls. The comparison between patients off medication and controls showed a weak trend toward longer latencies in the PD− group: 

-   PD+ vs. PD-: `r ttestPretty(subset(df_latency_subject, group == "PD+")$median_latency_ms, subset(df_latency_subject, group == "PD-")$median_latency_ms, var.equal = TRUE)`
-   Control vs. PD+: `r ttestPretty(subset(df_latency_subject, group == "Control")$median_latency_ms, subset(df_latency_subject, group == "PD+")$median_latency_ms, var.equal = TRUE)`
-   Control vs. PD-: `r ttestPretty(subset(df_latency_subject, group == "Control")$median_latency_ms, subset(df_latency_subject, group == "PD-")$median_latency_ms, var.equal = TRUE)`

To examine decision latency at the trial level, we fit a Bayesian hierarchical regression model predicting response time from group and distance to the previously selected tile, with a random intercept for participant (@tbl-latency-brms). Because response times were strictly positive and right-skewed, we used a lognormal link function. As expected, longer distances between consecutive clicks on the grid were associated with longer latencies. Controls responded faster than PD- patients, but the difference between PD patients on and off medication was small and the HDI included zero. The effect of distance on response time was slightly stronger in controls than in PD- patients, but the effect was small. 

Overall, these analyses suggest that medication status was not associated with a clear difference in response latency within the PD sample, and that latency variation was driven more strongly by search distance than by group.


```{r}
#| label: tbl-latency-brms
#| tbl-cap: "Hierarchical Bayesian regression: Median decision latency as function of group and distance to previously selected tile."



# multi-level regression
# dat <- dat %>% mutate(log_latency_ms = log(latency_ms))

# lmer_latency_reward_distance_group <- lmer(
#   log_latency_ms ~ previous_reward + distance + group + (1 | id),
#   data = filter(dat, round %in% 2:9, trial > 0)
# )
# 
# summary(lmer_latency_reward_distance_group)
# tab_model(lmer_latency_reward_distance_group)
# 
# emmeans(m1, pairwise ~ group, pbkrtest.limit = 20600)
# 
# lmer_latency_reward_distance_group <- lmer(
#   log_latency_ms ~ distance * group + (1 | id),
#   data = filter(dat, round %in% 2:9, trial > 0)
# )
# 
# summary(lmer_latency_reward_distance_group)
...
brm_latency_distance_group <- run_model(
  brm(
    latency_ms ~ distance * group + (1 | id),
    data = subset(dat, round %in% 2:9 & trial > 0 & !is.na(latency_ms)),
    family = lognormal(),
    cores = 4,
    chains = 4,
    seed = 815,
  ...
  modelName = "brm_latency_distance_group"
)
# summary(brm_latency_distance_group)
# tab_model(brm_latency_distance_group, transform="exp")
# tab_model(brm_latency_distance_group)

model_parameters(
  brm_latency_distance_group,
  centrality = "mean",
  ci = 0.95,
  ci_method = "hdi",
  digits = 3
...
Controls had lower judgment error than PD- patients; no other group differences were observed.
...
#| fig-cap: "Judgment error of bonus round judgments. The red dotted line indicates a random baseline."
#| fig-width: 6
#| fig-height: 5
...
p_bonusround_prediction_error <- ggplot(dat_bonus_error, aes(x = group, y = mean_error, color = group, fill = group, shape = group)) +
  ...
  scale_color_manual(values = groupcolors) +
  scale_fill_manual(values = groupcolors) +
  labs(title = "Reward judgment error", y = "Mean absolute error", x = "") +
  theme_classic() +
  theme(
    legend.position = 'none',
    legend.title = element_blank(),
...
ggsave("plots/S3_bonusround_judgment_error.png", p_bonusround_prediction_error, dpi=300, height = 5, width = 6 )

```

To address whether the variability in prediction error in the bonus round was related to disease severity, we ran a Bayesian linear regression in the PD sample predicting mean absolute error from medication group, BDI, MMSE, and Hoehn–Yahr score (@tbl-judgment-error-clinical-indicators). Posterior estimates for all predictors were small and their 95% HDIs included zero, including for Hoehn–Yahr score. Thus, we found no evidence that variability in judgment error was systematically related to disease severity or other clinical indicators.

```{r, echo=FALSE}
#| label: tbl-judgment-error-clinical-indicators
#| tbl-cap: "Bayesian regression: mean judgment error in the bonus round as a function of medication group and clinical indicators in the PD sample."

dat_bonus_error <- dat_bonus_error %>% 
  left_join(dat_sample, by = c("id", "group"))


brm_bonus_error_clinical_indicators <- run_model(brm(mean_error ~ group + hoehn_yahr + BDI + MMSE, 
                                                     data=dat_bonus_error, 
                                                     cores=4,  
                                                     chains=4,
                                                     backend = "cmdstanr",
                                                     control = list(adapt_delta = 0.99),
                                                     ...
                                                     warmup=1000, 
                                                     seed = 0815), 
                                                 modelName = 'brm_bonus_error_clinical_indicators')

model_parameters(brm_bonus_error_clinical_indicators,
                 centrality = "mean", ci = 0.95,
                 ci_method = "hdi" ) %>% 
  kable(digits = 3, format = "html")

# summary(brm_bonus_error_clinical_indicators, digits=3)

# for 3 digits estimates
# s <- summary(brm_bonus_error_clinical_indicators)
# 
# fixef_tab <- as.data.frame(s$fixed)
# fixef_tab$Estimate <- sprintf("%.3f", fixef_tab$Estimate)
# fixef_tab$Est.Error <- sprintf("%.3f", fixef_tab$Est.Error)
# fixef_tab$Q2.5 <- sprintf("%.3f", fixef_tab$Q2.5)
# fixef_tab$Q97.5 <- sprintf("%.3f", fixef_tab$Q97.5)
# 
# fixef_tab
# 
# spec_tab <- as.data.frame(s$spec_pars)
# spec_tab$Estimate <- sprintf("%.3f", spec_tab$Estimate)
# spec_tab$Est.Error <- sprintf("%.3f", spec_tab$Est.Error)
# spec_tab$Q2.5 <- sprintf("%.3f", spec_tab$Q2.5)
# spec_tab$Q97.5 <- sprintf("%.3f", spec_tab$Q97.5)
# 
# spec_tab

# tab_model(brm_bonus_error_clinical_indicators)

# get HDI
# x <- hdi(brm_bonus_error_clinical_indicators, 
#     paramaters = c("b_Intercept", "b_groupPDP", "b_hoehn_yahr", "b_BDI", "b_MMSE", "sigma"), 
#     component = "all",
#     digits = 3)
# as.data.frame(x)
# 
# df <- as.data.frame(x)
# df$CI_low  <- round(df$CI_low,  3)
# df$CI_high <- round(df$CI_high, 3)
# df

# analogous frequentist regression
# m <- lm(mean_error ~ group + hoehn_yahr + BDI + MMSE, data = subset(dat_bonus_error, group != "Control"))
# 
# summary(m)
# tab_model(m)
...
#### Judgment error and confidence
Aggregated across all judgments, there was no relationship between judgment error and confidence, with varying and inconsistent patterns in the different groups. The Control group showed a weak negative relation (i.e., lower confidence was associated with higher judgment error), the PD+ group a weak positive trend (i.e., lower confidence was associated with better lower judgment error), and the PD− group showed no association. None of these patterns were strong or reliable, no relationship between judgment error and confidence.
...
# Across all judgments and participants, there was no systematic relation between confidence and judgment error:
...
A Bayesian regression with  judgment error as dependent variable, and confidence and group and their interaction as population-level (“fixed”) effects, and a random intercept for participants yielded the same conclusions: in control patients, confidence and judgment error were weakly negatively related (i.e., lower confidence was associated with a higher error), whereas for the two Parkinson groups confidence ratings and judgment error were not related.
...
#tab_model(brm_bonus_confidence_error_by_group, bpe = "mean", title = "Bayesian regression results: judgment error and confidence") 

#bayes_R2(brm_bonus_confidence_error_by_group)

model_parameters(brm_bonus_confidence_error_by_group,
                 centrality = "mean", 
                 ci = 0.95,
                 ci_method = "hdi",
                 digits = 3) %>% 
...
ggplot(fixedDF_conf_error, aes(howSecure, error, color = group, fill = group)) +
  ...
  scale_y_continuous("Judgment Error") + #, breaks = seq(0,30,10)) +
  scale_x_continuous("Confidence", breaks = seq(0,10,1)) +
  coord_cartesian( xlim = c(0,10)) +
  ggtitle("Confidence and judgment error") +
  theme_classic() +
  theme(#aspect.ratio = 1,
    plot.title = element_text(size=16),
    legend.title = element_blank(),
...
ggsave("plots/S4_bonusround_regression_judgment_error_confidence.png", dpi=300, height = 4, width = 7 )
...
#| fig-cap: "Judged reward of chosen vs. not chosen options in bonus round."
#| fig-width: 9
#| fig-height: 4
...
p_bonusround_prediction_chosen <-
  ...
  scale_color_manual(values = groupcolors) +
  scale_fill_manual(values = groupcolors) +
  labs(title = "Judged reward of chosen vs. not chosen options", y = "Judged reward", x = "") +
  theme_classic() +
  theme(
    legend.position = 'none',
    legend.title = element_blank(),
...
p_bonusround_prediction_chosen
ggsave("plots/S5_bonusround_chosen_not_chosen_options_predicted_reward.png", p_bonusround_prediction_chosen, dpi=300, width=9, height = 4)
...
ggsave("plots/S6_model_comparison_R2.png", p_model_comparison_R2, dpi=300, width = 10, height = 5)
...
p_model_simulation_params
ggsave("plots/S7_model_simulation_params_lambda_0_5.png", p_model_simulation_params, bg="white", width = 14, height = 5, dpi = 300)
