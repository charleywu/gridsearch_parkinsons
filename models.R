...
bayesianMeanTracker <- function(x, y, theta=c(1), prevPost=NULL){ #Updates the previous posterior based on a single observation
  ...
  if (is.null(prevPost)){#if no posterior prior, assume it is the first observation
    predictions <- data.frame(mu=rep(mu0,gridSize), sig=rep(var0,gridSize))
...
nullModel <- function(..){ #
  predictions <- data.frame(mu=rep(1,gridSize), sig=rep(0,gridSize)) #defaults all means to zero
  return(predictions)
}
...
ucb<-function(out, pars, refactor=F){
  if (refactor==TRUE){
    ...
    outtotal<-matrix(outtotal, ncol=nrow(out)/gridSize, byrow=TRUE)
  }else{
    beta <- pars[1]
    #calulate all the upper confidence bounds
...
greedyMean <- function(out){
  ...
  outtotal[outtotal<0]<-0.0001
  outtotal[outtotal>50]<-50
  outtotal<-matrix(outtotal, nrow=nrow(out)/gridSize, byrow=TRUE)
  return(t(outtotal))
}
...
inertia<-function(blocks, trialnumber){
  ...
  predictions <- data.frame(mu=distance, sig = rep(0,gridSize))
