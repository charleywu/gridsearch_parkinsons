...
All participants answered the German version of the Beck Depression Inventory II, a self-report inventory consisting of 21 items measuring the severity of depression [@beck1996bdibeck; @hautzinger2006beck]. Each item is scored from 0 to 3, with higher values representing more severe symptoms, such that the total score ranges between 0 and 63.
...
Behavioral data are stored 

- *data_gridsearch_parkinson.csv*, which contains the behavioral data from rounds 1-9 from the task 
- *data_gridsearch_subjects.csv*, which contains participant information, and   
- *data_gridsearch_Parkinson_bonusround.csv*, which contains the behavioral data from round 10 from the task ("bonus round"). 

Data frame _dat_ includes all info from _data_gridsearch_parkinson.csv_ and _data_gridsearch_subjects.csv_, with the following variables (@tbl-dat):
...
### No performance differences across rounds or clinical indicators

@fig-performance-rounds shows the obtained rewards by round, for each group. 
...
A Bayesian regression analysis yielded comparable results, with the estimated effect of round on mean reward being very small and the credible interval including zero (@tbl-performance-rounds-brm). In the all subsequent analyses, we therefore average across rounds.
...
A Bayesian regression yielded comparable results (@tbl-performance-clinical-predictors-PD-brms).
```{r}
#| label: tbl-performance-clinical-predictors-PD-brms
#| tbl-cap: "Hierarchical Bayesian regression with random intercept: Reward as function of BDI-II, MMSE, and Hoehn-Yahr score (only PD patients)."
#| 
...
## Bonus round judgments {#sec-bonusround}
...
ggsave("plots/SI_bonusround_prediction_error.png", p_bonusround_prediction_error, dpi=300, height = 5, width = 6 )
...
ggsave("plots/SI_bonusround_regression_prediction_error_confidence.png", dpi=300, height = 4, width = 7 )

```

### Analysis of selected tiles

To analyze selected and not-selected options, we first averaged the predicted reward and confidence of the not-chosen tiles within subjects, and then compared chosen and not chosen options. 
...
p_bonusround_prediction_chosen
ggsave("plots/SI_bonusround_chosen_not_chosen_options_predicted_reward.png", p_bonusround_prediction_chosen, dpi=300, width=9, height = 4)
...
#| fig-cap: "Reward predictions in bonus round."
#| fig-width: 15
#| fig-height: 4
#| 
...
#### Chosen vs. not chosen tiles: True rewards
There was no difference in terms of true underlying reward between selected and not selected tiles in any of the groups.

-   Control: `r ttestPretty(subset(df_chosen_group_wide_true_rewards, group == 'Control')$chosen, subset(df_chosen_group_wide_true_rewards, group == 'Control' )$not_chosen, paired = T)`
-   PD+: `r ttestPretty(subset(df_chosen_group_wide_true_rewards, group == 'PD+')$chosen, subset(df_chosen_group_wide_true_rewards, group == 'PD+' )$not_chosen, paired = T)`
-   PD-: `r ttestPretty(subset(df_chosen_group_wide_true_rewards, group == 'PD-')$chosen, subset(df_chosen_group_wide_true_rewards, group == 'PD-' )$not_chosen, paired = T)`
...
ggsave("plots/behavioral_results.png", dpi=300, height=15, width=18)
ggsave("plots/behavioral_results.pdf", height=15, width=18)
...
#   group_by(group,hoehn_yahr) %>% 
#   summarise(mean_reward = mean(mean_reward))
