...
//                    ERIC SCHULZ, CHARLEY WU                         //
//          Refactoring and app version: Federico Meini               //
////////////////////////////////////////////////////////////////////////
...
function assignScenarioSkipRegistration() {
  ...
  //gender = isrcUtils.GetUriParam('gender');
  //age = isrcUtils.GetUriParam('age') ? isrcUtils.GetUriParam('age') : 0;
  //grade = isrcUtils.GetUriParam('grade') ? isrcUtils.GetUriParam('grade') : 0;
...
function assignScenario() {
  //Extract gender and birthdate
	...
	var dob = "Na";
  	session = 1;//parseInt(document.getElementById('sessionSelect').value);
...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  // skip registration page if uid and age are in the URL
  if (isrcUtils.GetUriParam('uid') != null && isrcUtils.GetUriParam('age') != null) {
    assignScenarioSkipRegistration();
  }
  else {
...
function onCellTapped(cell) {
  ...
  change('scoretotal', "Rundenkonto: " + scoretotal[trialCounter]);
  ...
  if (clicks == 0) {
    ...
    completionDiv = "<br><br><br><br><h1 class=\"text-xl\">Du hast dieses Feld erkundet und hast <b>" 
      + starRating + " Punkte gesammelt</b>. Du hast noch " + (trials) + " " + remainingMsg + "</h1><br><br><br>" 
      + starDiv + "<br><br><br>";
  ...
  if (trials == 0 && clicks == 0) {
      ...
      completionDiv = "<h1 class=\"text-xl\"> Du hast insgesamt ganze <b>" + finalStarCount + " Punkte gesammelt</b>! Vielen Dank fürs Spielen!</b> </h1>" + finalStarDiv + "";
...
function senddata() { 
  //search history
 // participantId=document.getElementById("uid-input")
 // Session=document.getElementById("gradeSelect")
