...
packages <- c('tidyverse', 'cowplot', 'viridis', 'colorspace', 'scales')
...
sim = read.csv('modelResults/simulatedModels_local.csv')
...
sim_means <-  sim %>% 
  ...
  mutate(log10_beta = log10(beta),
         log10_tau = log10(tau)
  )

# choose nice tick positions in the original scale
bx <- c(0.001, 0.01, 0.1, 1, 10, 50)
by <- c(0.001, 0.01, 0.1, 1, 10, 20)

ggplot(sim_means, aes(x = log10_beta, y = log10_tau, fill = meanReward)) +
  geom_raster() +
  scale_x_continuous(breaks = log10(bx), labels = bx, expand = c(0,0)) +
  scale_y_continuous(breaks = log10(by), labels = by, expand = c(0,0)) +
  labs(x = expression(paste('Explorations bonus ', beta)),
       y = expression(paste('Random exploration ', tau))) +
  scale_fill_gradientn(colours = colorspace::sequential_hcl(500, "Inferno"),
                       name = "Normalized\nreward") +
  theme_classic() +
...
ggsave("plots/model_simulation_local.png", dpi = 300, width = 12, height = 6)
# 
# ################################################################################################
